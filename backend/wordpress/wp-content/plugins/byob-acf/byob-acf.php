...
function byob_block_render_callback( $block ) {
	...
	$attributes_encoded = rawurlencode( wp_json_encode( $attributes ) );

	if ( ! empty( $frontend_url ) ) : ?>
		<div style="position: relative; width: 100%; height: 100%; pointer-events: none;">
			<iframe
				class="blockPreviewIframe"
				src="<?php echo( esc_url( $frontend_url . '/block-preview?name=' . $block['name'] . '&acfAttributes=' . $attributes_encoded ) ); ?>"
				title="<?php echo( esc_attr( $block['name'] ) . ' Preview' ); ?>"
			></iframe>
		</div>
		<script>
